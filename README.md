...
A physical Pedi-Wheel exists, but is large and may not be as readily available as a cellphone would be. Also the information from the physical Pedi-Wheel is often outdated and requires the purchase of the current product. [Here is the actual Pedi-Wheel in action](https://vimeo.com/8114904). These features and more make the Pedi-Wheel a strong candidate for progressive web applications treatments.
...
- Notes - View actual drug dose ranges and drug-specific notes
- Cache-busting - Save the data pulled from the updateable manifest file so the application can run faster by not downloading new data each time it is accessed
- Offline usage - Access the application without a network connection
- The application takes advantage of browser APIs giving access to device information, such as accelerometer and GPS
- CSS - Leverage modern grid and flexbox techniques
